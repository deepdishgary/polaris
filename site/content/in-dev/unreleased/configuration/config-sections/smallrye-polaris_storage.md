...
| `polaris.storage.cloudflare-r2.access-key` |  | `string` | Access key id of the default Cloudflare R2 parent API token. The parent token must carry  Object Read and Write permission on every bucket the catalogs use; a temporary credential  cannot exceed it.  |
| `polaris.storage.cloudflare-r2.secret-key` |  | `string` | Secret access key of the default Cloudflare R2 parent API token.  |
| `polaris.storage.cloudflare-r2.`_`<storage>`_`.access-key` |  | `string` | Access key id of the Cloudflare R2 parent API token for catalogs with this `storageName` .  |
| `polaris.storage.cloudflare-r2.`_`<storage>`_`.secret-key` |  | `string` | Secret access key of the Cloudflare R2 parent API token for catalogs with this `storageName` .  |
