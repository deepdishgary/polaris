 ...
 *
 * <p>The server under test needs the R2 parent token in {@code
 * polaris.storage.cloudflare-r2.access-key} and {@code polaris.storage.cloudflare-r2.secret-key}.
 * The launched server inherits this JVM's environment, so exporting {@code
 * POLARIS_STORAGE_CLOUDFLARE_R2_ACCESS_KEY} and {@code POLARIS_STORAGE_CLOUDFLARE_R2_SECRET_KEY} is
 * the only supported way to supply it. Never pass the parent token as a Quarkus config override:
 * for {@link QuarkusIntegrationTest} every override becomes a {@code -Dkey=value} argument on the
